	...
	else //if (!ent.isChild) print those not a child
	{
		_levelEditor->LE_AddSelectable(name_with_index, LE_ECHELPER->SelectedEntities()[ent.getId()],
			[&ent, this]()
			{
				...
				std::string outStr = "HierarchyInspector::EntityFunction: ";  outStr.append(std::to_string(ent.getId())).append(". has been selected: ").append(std::to_string(LE_ECHELPER->SelectedEntities()[ent.getId()]));
